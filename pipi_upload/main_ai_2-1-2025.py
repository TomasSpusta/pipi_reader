...
async def handle_user_session(
    ...
    try:
        ...
        async def prolong_reservation(
            ...
            new_card_id = await asyncio.wait_for(rfid_reader.read_card(), timeout=5)
        ...
        async def monitor_and_update():
            ...
            async with asyncio.TaskGroup() as tg:
                tg.create_task(monitor_button(
                    button, lcd_controller, session, instrument,token ))
                tg.create_task(update_session_time(lcd_controller, session, token))
                #tg.create_task(prolong_reservation(card_id, user, lcd_controller, token, session))
        ...
        print (f"session.remaining_time is {session.remaining_time}")
...
async def update_session_time(
    ...
    while session.remaining_time > 0:
        await lcd_controller.message ("Recording is running", f"Remaining time: {session.remaining_time}", backlight=False)
        session = await networking.fetch_reservation_info(token, session)
        print (f"Update_session_time: {session.remaining_time}")
        await asyncio.sleep(5)
    print (f"Update_session_time_outside loop: {session.remaining_time}")
    ...
    while True:
        if button.is_pressed:  # Replace with actual button press detection
            ...
            if hold_time >= delay:
                ...
                print (f"Button Stop recording session time: {session.remaining_time}")
