                        ...
                        if user:
                            ...
                            if session:
                                ...
                                while session.remaining_time > 0:
                                    print(
                                        f"session loop... remaining time {session.remaining_time} "
                                    )
                                    async with asyncio.TaskGroup() as tg:
                                        task1 = tg.create_task(
                                            lcd_controller._write(
                                                text=f"Remaing session time{session.remaining_time} minutes",
                                                row=2,
                                            )
                                        )
                                        task2 = tg.create_task(
                                            monitor_button(button, lcd_controller)
                                        )
                                        task3 = tg.create_task(
                                            networking.fetch_reservation_info(
                                                token, session
                                            )
                                        )
                                    ...
                                    await lcd_controller.message(
                                        line1="Recording is ended", display_time=5
                                    )
